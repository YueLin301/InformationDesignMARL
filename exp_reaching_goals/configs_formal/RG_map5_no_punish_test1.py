...
config.main.exp_name = 'RG_map5_no_punish_test1'
...
config.train.n_episodes = 250000
...
config.sender.sender_objective_alpha = 0.01
config.sender.sender_constraint_right = 5
config.sender.coe_for_recovery_fromgumbel = 2
if config.sender.gaussian_distribution:
    config.sender.gaussian_var = 2
config.sender.epsilon_greedy = 0.05
config.sender.epsilon_decay = 0.99999  # after generating every episode: epsilong <- epsilon * coe_decay
config.sender.epsilon_min = 0.01
...
config.receiver.gamma = 0.2
# config.receiver.entropy_coe = 1e-4
config.receiver.entropy_coe = 0.01
