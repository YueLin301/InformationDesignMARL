...
class pro_formal_constrained():
    def __init__(self, config, device):
        ...
        self.message_table = torch.tensor([0, 1], dtype=torch.float32)
    ...
    def update_infor_design(self, buffer):
        ...
        ''' BCE Obedience Constraint (regularizer) '''
        pi = buffer.a_prob_hr
        ...
        if constraint_left >= self.config.pro.constraint_right:
            ...
            gradeta_flatten = gradeta_flatten
        else:
            gradeta_flatten = self.config.pro.sender_objective_alpha * gradeta_constraint_flatten
