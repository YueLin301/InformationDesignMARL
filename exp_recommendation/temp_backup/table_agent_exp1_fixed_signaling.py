...
sender_objective_alpha = 0  # 这是拉格朗日的那个lambda

# train_n_episodes = 5e3
train_n_episodes = 1e4
# train_n_episodes = 3e4
# train_n_episodes = 5e4
...
class pro_class():
    def __init__(self):
        ...
        if config_env.initialize:
            set_net_params(self.critic, params=config_env.critic_pro_params.clone())
            set_net_params(self.signaling_net, params=config_env.signaling_params.clone())
    ...
    def calculate_true_constraint_left(self):
        # 这样没梯度。。。
        prob_good = config_env.prob_good
        prob_bad = 1 - prob_good

        input_0 = torch.tensor([1, 0], dtype=torch.double)
        input_1 = torch.tensor([0, 1], dtype=torch.double)

        _, phi_when_bad, _ = self.send_message(input_0)
        _, phi_when_good, _ = self.send_message(input_1)

        _, pi_when_signal0, _ = self.hr.choose_action(input_0)
        _, pi_when_signal1, _ = self.hr.choose_action(input_1)

        pi = [pi_when_signal0, pi_when_signal1]

        constraint_left_all = 0
        for signal in range(2):
            for a in range(2):
                constraint_left_i = prob_bad * phi_when_bad[signal] * pi[signal][a] \
                                    * (config_env.rewardmap_HR[0][a] - config_env.rewardmap_HR[0][1 - a]) \
                                    + prob_good * phi_when_good[signal] * pi[signal][a] \
                                    * (config_env.rewardmap_HR[0][a] - config_env.rewardmap_HR[0][1 - a])
                if constraint_left_i >= 0:
                    constraint_left_all += constraint_left_i

        return constraint_left_all
...
class hr_class():
    def __init__(self):
        ...
        if config_env.initialize:
            set_net_params(self.critic, params=config_env.critic_params.clone())
            set_net_params(self.actor, params=config_env.actor_params.clone())
