...
config_env.howoften_update = 5  # 几个episode更新一次
...
train_n_episodes = 1.5e5
# train_n_episodes = 2.5e5
...
class pro_class():
    ...
    def update_infor_design(self, buffer):
        ...
        for transition in buffer:
            ...
            r_couterfactual = config_env.rewardmap_HR[obs_int][a_couterfactual]
        ...
        if constraint_left > constraint_right:
            # gradeta_constraint_flatten = (r_hr - r_couterfactual) * (
            #         pi_at * gradeta_phi_sigma_flatten * self.temperature
            #         + phi_sigma * gradeta_pi_at_flatten
            # )
            gradeta_constraint = torch.autograd.grad(constraint_left, list(self.signaling_net.parameters()),
                                                     retain_graph=True)
            gradeta_constraint_flatten = self.flatten_layers(gradeta_constraint, dim=1)
...
class hr_class():
    ...
    def update_ac(self, buffer, calculate_for_upper_level=True):
        critic_loss = 0
        actor_obj = 0
        for transition in buffer:
            ...
            if calculate_for_upper_level:
                ...
                self.lower_level_critic_loss = critic_loss_i.detach()
